...
# Max chunk size for base64 transfers (in chars)
MAX_CHUNK_SIZE=900
...
if [ "$USE_LOCAL_CHART" = true ]; then
    ...
    # Base64 encode the chart package for transfer - save in a file for reliability
    log "Encoding chart package for transfer..."
    base64 "$CHART_PACKAGE" > "$TEMP_DIR/chart.b64"
    
    # Count the number of lines in base64 file to assist with chunking
    NUM_BASE64_LINES=$(wc -l < "$TEMP_DIR/chart.b64")
    log "Chart encoded as base64 (${NUM_BASE64_LINES} lines)"
fi
...
spec:
  ...
  restartPolicy: Never"
    ...
    # This time using a file transfer approach for the chart
...
echo 'DEBUG: Setting up chart transfer...'
# Using multipart transfer via kubectl cp
# First create a local copy of the chart package
cat > /tmp/chart-package.tgz.b64 << 'BASE64_CHART_EOF'
EOF
)

    # Append the base64 chart data using cat
    DEPLOY_COMMAND+="$(cat "$TEMP_DIR/chart.b64")"
    DEPLOY_COMMAND+=$'\n'"BASE64_CHART_EOF"$'\n'

    # Continue the deployment command
    DEPLOY_COMMAND+=$(cat <<EOF
# Decode the base64 chart
base64 -d /tmp/chart-package.tgz.b64 > /tmp/chart-package.tgz
if [ \$? -ne 0 ]; then
...
EOF
    ...
    # Use Helm directly in the AKS command environment with file transfer for the chart
    log "Creating deployment command using direct Helm execution..."
    
    # Create the first part of the deployment command
    DEPLOY_COMMAND=$(cat << EOF
#!/bin/bash
set -e
...
echo 'Transferring chart package via base64 multipart...'
# Create the chart package file
touch /tmp/chart-deploy/chart-package.tgz.b64
EOF
)

    # Now add the base64 encoded chart in chunks - create a new temporary file for this
    # to avoid shell command length limitations
    CHART_B64_CHUNKS="$TEMP_DIR/chart_chunks.sh"
    
    # Write the header to the chunks file
    echo '# Chart chunks appended here' > "$CHART_B64_CHUNKS"
    
    # Split the base64 content into manageable chunks to avoid shell limitations
    # Read the base64 content in chunks and create append commands
    chunk_num=0
    while read -r chunk; do
        if [ -n "$chunk" ]; then
            # Escape any single quotes in the chunk
            chunk_escaped=$(echo "$chunk" | sed "s/'/'\\\\''/g")
            echo "cat >> /tmp/chart-deploy/chart-package.tgz.b64 << 'EOF_CHUNK_${chunk_num}'" >> "$CHART_B64_CHUNKS"
            echo "$chunk" >> "$CHART_B64_CHUNKS"
            echo "EOF_CHUNK_${chunk_num}" >> "$CHART_B64_CHUNKS"
            ((chunk_num++))
        fi
    done < <(fold -w "$MAX_CHUNK_SIZE" "$TEMP_DIR/chart.b64")
    
    log "Created $chunk_num chunks for base64 chart transfer"
    
    # Add the chunks script to the deployment command
    DEPLOY_COMMAND+=$'\n'"$(cat "$CHART_B64_CHUNKS")"$'\n'
    
    # Continue with the rest of the deployment command
    DEPLOY_COMMAND+=$(cat << EOF

echo "DEBUG: Base64 chart package transferred in $chunk_num chunks"
echo "DEBUG: Base64 file size check:"
ls -la /tmp/chart-deploy/chart-package.tgz.b64
wc -l /tmp/chart-deploy/chart-package.tgz.b64

# Now decode the base64 file
echo "DEBUG: Decoding base64 chart package..."
cat /tmp/chart-deploy/chart-package.tgz.b64 | base64 -d > /tmp/chart-deploy/chart-package.tgz
DECODE_RESULT=\$?
...
if [ \$DECODE_RESULT -ne 0 ]; then
  ...
  head -5 /tmp/chart-deploy/chart-package.tgz.b64
  exit 1
fi
