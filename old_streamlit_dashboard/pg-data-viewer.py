...
def render_data_viewer():
    ...
    with tabs[3]:
        ...
        with st.expander("Database Maintenance"):
            ...
            These operations can help improve database performance by rebuilding indices
            ...
            if st.button("Run VACUUM"):
                try:
                    ...
                    if not conn:
                        ...
                        with conn.cursor() as cur:
                            # Set statement timeout to avoid long-running operations
                            cur.execute("SET statement_timeout = 300000")  # 5 minutes
                            
                            st.info("Running VACUUM ANALYZE... This may take a few minutes.")
                            cur.execute("VACUUM ANALYZE sentiment_results")
...
# SQL Schema for sentiment_results table
def create_table_schema():
    """Create sentiment_results table if it doesn't exist"""
    try:
        conn = get_postgres_conn()
        if not conn:
            return False
        ...
        with conn.cursor() as cur:
            ...
            if not table_exists:
                ...
                # Create indices
                cur.execute("CREATE INDEX idx_sentiment_timestamp ON sentiment_results(timestamp)")
                cur.execute("CREATE INDEX idx_sentiment_ticker ON sentiment_results(ticker)")
                cur.execute("CREATE INDEX idx_sentiment_source ON sentiment_results(source)")
                
                conn.commit()
                print("Created sentiment_results table and indices")
...
def insert_sample_data():
    """Insert sample data into sentiment_results table"""
    try:
        conn = get_postgres_conn()
        if not conn:
            return
        ...
        sources = ['news', 'reddit', 'twitter', 'sec']
        
        # List of sample models
        models = ['finbert', 'fingpt', 'llama4']
        
        # Sample data - Generate 1000 records for demonstration
        now = datetime.now()
        ...
        with conn.cursor() as cur:
            for i in range(1000):
                ...
                sentiment = (np.random.random() * 2) - 1
                
                # Random confidence (0.5 to 1.0)
                confidence = 0.5 + (np.random.random() * 0.5)
                
                # Article ID
                article_id = f"{source[0].upper()}{i:06d}"
                
                # Article title
                article_title = f"{ticker} {'rises' if sentiment > 0 else 'falls'} on {'positive' if sentiment > 0 else 'negative'} {source} sentiment"
                ...
                (timestamp, ticker, sentiment, confidence, source, model, article_id, article_title)
            ...
            conn.commit()
            print(f"Inserted {i+1} sample records")
...
# Main function
def main():
    """Main entry point for the data viewer application"""
    # Setup page config
    st.set_page_config(
        page_title="RT Sentiment Data Explorer",
        page_icon="📊",
        layout="wide",
