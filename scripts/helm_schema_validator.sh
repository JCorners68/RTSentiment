...
validate_values_against_schema() {
  ...
  if check_command "jsonschema"; then
    ...
    if check_command "yq"; then
      ...
      # Fallback - skip validation since we can't convert YAML to JSON properly
      log "WARNING" "Cannot perform full schema validation without yq"
      
      # Basic validation check
      if jq -e 'true' "${VALUES_FILE}" 2>/dev/null; then
        log "INFO" "Values file is valid YAML/JSON"
      else
        log "ERROR" "Values file is not valid YAML/JSON"
        return 1
      fi
    fi
  else
    ...
    # Basic validation using jq (limited functionality)
    if jq -e 'true' "${VALUES_FILE}" 2>/dev/null; then
      log "INFO" "Values file is valid YAML/JSON"
    else
      log "ERROR" "Values file is not valid YAML/JSON"
...
# Validate security contexts are properly placed
